...
10. [환경 변수](#환경-변수)
11. [명령어 목록](#명령어-목록)
12. [업데이트 내용 요약](#업데이트-내용-요약)
13. [파일·문서 위치](#파일문서-위치)
14. [워크플로우](#워크플로우)
15. [아키텍처](#아키텍처)
16. [보안·데이터 흐름](#보안데이터-흐름)
17. [문제·오류 대처](#문제오류-대처)
18. [FAQ](#faq)
19. [법률·저작권·라이선스·상업적 용도](#법률저작권라이선스상업적-용도)
...
**몰라도 됩니다 — 전부 선택 사항입니다.** 아래는 특수한 상황(다른 컴퓨터로 옮기거나, 자동 안내를 끄고 싶을 때)에만 필요합니다.

| 환경 변수 | 하는 일 | 기본값 | 언제 쓰나 |
|---|---|---|---|
| `SODAM_GRAPH_SILENT` | `1`로 설정하면 세션 시작 시 자동 3줄 안내를 끕니다 | 없음(안내 켜짐) | 안내가 방해될 때 |
| `SODAM_GRAPH_SEARCH_ROOTS` | 형제 프로젝트를 찾을 폴더 경로를 `data/graph.json`의 값 대신 여기서 지정합니다(콤마로 여러 경로 구분) | `data/graph.json`의 `config.search_roots` | 다른 컴퓨터로 옮기거나, 저장소 파일에 실제 PC 경로를 남기고 싶지 않을 때 |
| `SODAM_GRAPH_DEBUG` | `1`로 설정하면 판정·발행 과정의 상세 로그를 출력합니다 | 없음(조용히 실행) | 동작이 이상해서 원인을 봐야 할 때 |
| `SODAM_GRAPH_ROOT` | 🔧 내부 테스트 전용 — 일반 사용자는 쓸 일이 없습니다 | 없음 | 개발자가 격리 테스트할 때만 |

**설정 방법 (Windows PowerShell 예시)**:
```
$env:SODAM_GRAPH_SILENT = "1"
```
이렇게 하면 **그 터미널 창에서만** 적용됩니다. 항상 적용하려면 Windows 시스템 환경 변수에 등록하세요.
...
| `/graph-block` | 사람 몫 블로커를 기록·해제 | AI가 대신 못 하는 일 때문에 막혔을 때 |
...
<details><summary><code>/graph-verify</code> (실제 출력 — 성공·실패 3가지)</summary>

```
> /graph-verify sodam-graph-eng.M2   (성공했을 때)
검증 완료 — sodam-graph-eng.M2 는 verified 로 확정됐습니다 (3135ms).

> /graph-verify sodam-graph-eng.M0   (이미 검증 단계를 지났을 때)
실패 — 이미 "done" 상태입니다 — 자동 검증은 todo/doing 에서만 verified 로 올립니다

> /graph-verify sodam-loop-eng.current   (검증 대상이 아닐 때 — 형제 마일스톤이거나 확인 방법이 자동화돼 있지 않음)
실패 — 자동 검증 대상이 아닙니다(자기 소유·배열형 verify_cmd 필수)
```
</details>

<details><summary><code>/graph-block</code> (사람 몫 블로커 기록·해제, 실제 출력)</summary>

```
> /graph-block sodam-loop-eng.current --kind human_test --description "새 세션 라이브 확인"   (기록)
기록 완료 — blk-001 (2026-08-21부터, sodam-loop-eng.current)

> /graph-block --resolve-block blk-001   (풀렸을 때 해제)
해제 완료 — blk-001 (sodam-loop-eng.current)

> /graph-block sodam-loop-eng.current --kind bogus --description "x"   (종류를 잘못 적었을 때 — 허용되는 5가지 중 하나만 가능)
실패 — 허용되지 않는 kind "bogus" — human_test·legal·external_account·decision·purchase 중 하나

> /graph-block no-such-milestone --kind legal --description "x"   (없는 마일스톤 ID를 적었을 때)
실패 — 없는 마일스톤 "no-such-milestone"
```

기록한 블로커는 `/graph-why` 결과 맨 끝에 **오래 기다린 순**으로 자동으로 뜹니다:

```
사람 몫 블로커 1건 (오래 기다린 순):
  · 소담루프엔지니어링 · 진행 단계 거친 판정 (마일스톤 상세 미추출) — 0일째 (human_test)
     └ 새 세션 라이브 확인   [blk-001]
```
</details>
...
<details>
<summary><b>v0.2.1 (2026-09-01) — 판정 정확도 버그 수정 + Mermaid 렌더링 실사용 확인 완료</b></summary>

- 🔧 **버그 수정**: 형제 6곳 중 5곳(하네스·컨텍스트·에이전틱·루프·리버스)에서 "며칠째 멈춰있나" 판정이 실제 git 활동일이 아니라 최초 측정일에 고정돼 최대 36일까지 틀려있던 문제를 발견·수정했습니다. 각 저장소의 실제 커밋 기록과 직접 대조해 바로잡았습니다. 소담프롬프트엔지니어링은 원래 값이 정확해 손대지 않았습니다.
- ✅ **`/graph-map`(Mermaid 그림) 실사용 확인 완료**: GitHub·VS Code 양쪽에서 실제로 그림이 렌더링되는 것을 사람이 직접 눈으로 확인했습니다(이전엔 코드만 확인된 상태였습니다).
- 이 두 가지로 **1차 로드맵(Phase 1+2)이 전부 완료**됐습니다. 사용자가 직접 켜고 끄는 명령어(사용법)는 이번 버전에서 바뀐 게 없습니다.

</details>

<details>
<summary><b>v0.2.0 (2026-08-21) — 더 정확하게, 사람 몫도 챙기게</b></summary>

- 완료 조건을 사람이 눈으로 안 봐도 **기계가 실제로 명령을 돌려서 확인**해주는 기능 추가 (`/graph-verify`)
- 다른 컴퓨터로 옮기거나 폴더 위치가 바뀌어도 **설정값 하나로 다시 찾게** 만드는 기능 추가(개발자용 옵션)
- 진행 기록 파일(`data/graph.json`)이 **깨지거나 손상됐을 때** 이상한 결과를 내는 대신 **바로 알려주고 멈추도록** 안전장치 추가
- **사람만 할 수 있는 일 때문에 막혔을 때**, 그걸 기록해두고 "며칠째 대기 중"인지 자동으로 보여주는 기능 추가 (`/graph-block`, `/graph-why` 결과 맨 끝에 표시됨)
...
아침에 Claude Code 를 켠다
  ...
  → 뭐부터 할지 모르면 /graph-why (사람 몫 블로커도 여기 같이 뜬다)
  → 전체를 보고 싶으면 /graph-next
  → 완료 조건을 기계가 바로 확인할 수 있는 항목이면 /graph-verify
  → 완료 제안이 뜨면 맞으면 두고, 틀리면 /graph-reject
  → 사람만 할 수 있는 일 때문에 막히면 /graph-block 으로 기록, 풀리면 다시 /graph-block 으로 해제
```
...
| `/graph-block`·`/graph-verify` 로 직접 쓰는 것도 안전한가요? | **예.** 이 프로그램 자신의 `data/graph.json` 안에만 씁니다. 형제 폴더는 여전히 읽기만 하고, 동시에 여러 창에서 써도 순서대로 안전하게 처리됩니다(잠금 장치) |
...
**17-26. `/graph-verify` 실행 시 `실패 — 자동 검증 대상이 아닙니다`**
원인: 형제 프로젝트의 마일스톤이거나, 그 항목의 확인 방법이 아직 자동화돼 있지 않음(사람이 직접 봐야 함). 해결: **정상 동작입니다** — 사람이 직접 확인해주세요.

**17-27. `/graph-verify` 실행 시 `실패 — 이미 "○○" 상태입니다`**
원인: 이미 검증 단계를 지나 다음 단계로 넘어간 항목. 해결: 다시 실행할 필요가 없습니다.

**17-28. `/graph-verify` 실행 시 `실패 — 검증 실패 — ...`**
원인: 실제로 확인 명령을 돌려봤는데 조건을 못 채움. 해결: 메시지에 적힌 원인 그대로 사람에게 전달해주세요 — 실제로 아직 안 끝난 것입니다.

**17-29. `/graph-block` 실행 시 `실패 — 허용되지 않는 kind`**
원인: 블로커 종류는 `human_test`(사람이 직접 하는 테스트) · `legal`(법무) · `external_account`(외부 계정) · `decision`(결정) · `purchase`(구매) 5가지뿐입니다. 해결: 이 중 하나로 다시 입력하세요.

**17-30. `/graph-block` 실행 시 `실패 — 없는 마일스톤`**
원인: 마일스톤 ID 오타. 해결: `/graph-where`나 `/graph-next`로 정확한 ID를 먼저 확인하세요.

**17-31. `/graph-block --resolve` 실행 시 `실패 — 없는 블로커 id`**
원인: 이미 해제됐거나 ID를 잘못 입력함. 해결: `/graph-why` 출력에서 현재 남아 있는 블로커 목록을 다시 확인하세요.
...
10. **저장하는 정보에 비밀번호가 들어가나요?** → 아니요. 다만 `/graph-block`으로 직접 적는 "무엇을 해야 풀리나" 같은 설명 칸은 사람이 쓰는 자유 텍스트라, 여기에 비밀번호나 개인정보를 직접 적으면 그대로 저장됩니다 — 일반 텍스트만 적어주세요.
11. **`/graph-verify`는 뭘 확인하나요?** → 마일스톤마다 정해둔 "확인 명령"을 실제로 실행해서 통과하면 자동으로 다음 단계로 올려줍니다. 이 프로그램 자기 자신의 항목에만 쓸 수 있고, 실행 가능한 명령도 미리 허용된 몇 개뿐입니다.
12. **`/graph-block`은 뭘 하는 건가요?** → "사람만 할 수 있는 일이라 AI가 대신 못 하는 것"을 기록해두는 기능입니다. 기록하면 `/graph-why`에 "며칠째 대기 중"으로 자동 표시되고, 풀리면 다시 명령으로 지워야 합니다(안 지우면 계속 대기 중으로 남습니다).
13. **상업적으로 써도 되나요?** → **됩니다**(Apache-2.0, 아래 [법률](#법률저작권라이선스상업적-용도) 참조).
14. **문제가 생기면 어디에 물어보나요?** → 저장소 이슈로 남겨주세요.
15. **새 버전이 나왔는데 어떻게 받나요?** → `/plugin uninstall sodam-graph@sodamgraph-marketplace` 후 `/plugin install sodam-graph@sodamgraph-marketplace`로 다시 설치하고 Claude Code를 완전히 재시작하세요. `/plugin install`만 다시 누르면 "이미 설치됨"이라고만 뜨고 최신 내용을 안 받아오는 경우가 실제로 확인됐습니다 — 제거 후 재설치가 확실한 방법입니다.
...
> ⚠️ **AI 생성물 사용 전 확인 권장**: AI 생성 코드·문서의 저작권 귀속·유사 저작물 침해 가능성은 국가·시기별로 **법리가 아직 정립되는 중**입니다. **상업적으로 사용하거나 제3자에게 납품하기 전에는** 저작권·출처·상업적 이용 가능 여부·유사 저작물 침해 가능성을 사용자가 직접 확인하는 것을 권장합니다. 이 프로젝트는 그 결과를 보장하지 않습니다.
