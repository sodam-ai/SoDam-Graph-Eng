...
이 저장소는 실제로 PRIVATE 상태입니다. 그래도 위 (A) 방법이 됩니다(직접 실측 확인):
...
```
data/graph.json         정본 — 형제 목록·계획 (git 으로 관리됨)
data/snapshot.json       실측 캐시 (스캔할 때마다 갱신, 저장 안 함)
data/shadow/              사람이 채워준 정보
docs/                     형제 프로젝트에 제출하는 문서(작성만, push는 사람 몫)
~/.sodam/graph-state.json  다른 형제가 읽어가는 요약본 (내 컴퓨터의 사용자 폴더 안)
```
...
12. **문제가 생기면 어디에 물어보나요?** → 저장소 이슈(비공개 저장소라 접근 권한이 있는 분만 가능)로 남겨주세요.
...
| 책임 제한 | "있는 그대로" 제공되며 보증이 없습니다. 써서 생긴 손해는 사용자 책임입니다 |
| 외부 자료 주의 | 이 프로젝트 자체는 외부 패키지를 쓰지 않습니다(Node.js 기본 기능만) |

> ⚠️ 이 절은 법률 자문이 아닙니다. 참고용이며 법적 효력을 보장하지 않습니다.
