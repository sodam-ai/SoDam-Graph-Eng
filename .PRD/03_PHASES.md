...
- [ ] **M-W — 배선 보수 (14차 신설) ★최우선**
  ...
  - `done_when` (**3개 전부**):
    ...
    2. **W-2** — 다음 할 일 **문장이 나오는 형제 ≥ 6/7** + 출력 문장이 형제 파일에 **그대로 존재**(grep 대조) + 전부 **`(추정` 표기** + `graph-state.json` 필드 **여전히 6개**
...
- [ ] **M8-A — 🔴 형제 계약 갱신 (14차 분리 — 앞으로 당김)**
  ...
  - `done_when` ①: **계약 갱신안 8건 작성 완료** (아래 표) — 🆕 **4번은 규약 F·붙여넣기 조각·`isFamilyAlive` 구현까지 포함** (`07` §5-B)
  - `done_when` ②: **불일치 7건 보고서** 작성 (`07` §1)
  - `done_when` ③: **읽는 쪽 실증 1건** — 형제 저장소를 고치지 않고, `07` §5-B 조각을 **`_test_fixture/` 안에서 실행**해 `graph-state.json` 을 정상으로 읽고 `stale` 판정까지 나오는 것을 확인
    - 🔴 **형제 저장소에는 한 글자도 쓰지 않습니다.** 갱신안은 **파일로 제출**하고 push는 사용자 몫 (읽기 전용 결정 불변)

- [ ] **M8-B — 문서 (README.md 한국어)**
...
- [ ] **`~/.sodam/graph-state.json` 이 🆕 자동으로 발행되고 다른 형제가 읽을 수 있다**
  - 🔴 **14차 강화**: *"발행된다"* 만으로는 부족합니다. **사람이 명령을 쳐야만 갱신되면 미구현**입니다 — 실제로 그 상태였습니다(발행본 22:33 vs 판정 23:56)
- [ ] 🆕 **다음 할 일이 문장으로 나오는 형제 ≥ 6/7** (`coarse` 여도 1.5층 인용 + `(추정)` 표기)
- [ ] 🆕 **읽는 쪽 계약(규약 F)과 붙여넣기 조각이 갱신안에 들어 있다** (`07` §5-B — 읽는 코드를 안 주면 아무도 안 읽습니다)
- [ ] 🆕 **`data/baseline-2026-08-02.json` 존재** (없으면 2주 성적표를 낼 수 없음)
...
★ 남은 것만: M-W → M11 → M9 → M8-A → M8-B  (M0~M7·M10 완료)
...
|-------|----------|-------------|----------|------|
| **Phase 1 (MVP, M0~M11 + M-W)** | 지도 + 다음 할 일 + **임계 경로 + 그림자 + done 자동 승격 + 공유 발행** | 경로 혼동 · 세션 재설명 · 정체 **(설계됨 — 검증 전)** | 없음 | 🟡 **진행 중** — M0~M7·M10 완료 · 남은 것 **M-W → M11 → M9 → M8-A → M8-B** |
