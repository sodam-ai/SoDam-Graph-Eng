...
> 🆕 **2026-09-12 — 위 A·B·D·E 블록이 `tools/verify-prd.sh`로 실행파일 추출됐습니다**(검사 로직 변경 없음, 매번 복붙 안 해도 `bash tools/verify-prd.sh` 한 줄로 실행). 이 문서의 블록은 **정본으로 그대로 보존**합니다 — 새 SPEC 항목을 추가할 때는 위 D) 목록과 스크립트 양쪽에 같은 줄을 더하십시오.
...
### 🆕 사전 설계 — `~/.sodam/loop-ack.json` (2026-09-01, 게이트 대기 중 미리 확정)

> **왜 미리 하는가**: `CHECKPOINT.md`가 이미 지적한 구조적 충돌 — M16 ①이 쓰려는 "읽음 신호"는 `graph-state.json`(S-6.1, 6필드 화이트리스트·Graph-Eng 단독 발행)에는 자리가 없습니다. 해법 이름(`loop-ack.json` 분리)만 정해져 있었고 스키마·읽기 규칙이 없었습니다. 이건 **문서 설계**이지 코드가 아니라서, 루프 게이트가 닫혀 있는 지금도 안전하게 확정할 수 있습니다. 게이트가 열리면 이 절을 그대로 구현 스펙으로 씁니다.
>
> **범위의 한계(정직하게 표시)**: Graph-Eng는 이 파일의 **소비자(reader)**일 뿐입니다. 파일을 실제로 쓰는 쪽은 소담루프엔지니어링이고, 그 프로젝트의 실제 쓰기 구현(원자적 교체 여부, 몇 초 간격으로 갱신할지 등)은 이 저장소가 대신 설계할 수 없습니다(S-3 읽기 전용 원칙 — 다른 저장소의 구현까지 이 문서가 못박으면 그건 이미 "간섭"입니다). 아래는 **Graph-Eng가 이 파일을 읽을 때 지킬 계약과 기대하는 스키마**까지만입니다.

| 항목 | 결정 |
|---|---|
| 경로 | `~/.sodam/loop-ack.json` (`graph-state.json`과 같은 디렉터리, 별개 파일) |
| 쓰는 주체 | 소담루프엔지니어링만. **Graph-Eng는 이 파일에 절대 쓰지 않음**(`safeWrite.mjs`의 `isWritable()`에 이 경로를 추가하지 않는다 — 추가하는 순간 S-3 위반) |
| 읽는 주체 | Graph-Eng(`lib/judge.mjs`)만, 읽기 전용 |

**기대 스키마(안)**:
```json
{
  "version": 1,
  "produced_by": "sodam-loop",
  "updated_at": "2026-09-01T00:00:00.000Z",
  "acks": [
    { "project_id": "sodam-loop-eng", "read_scanned_at": "<graph-state.json의 scanned_at 값>", "acked_at": "<ISO8601>" }
  ],
  "executing": [
    { "project_id": "sodam-loop-eng", "milestone_id": "<graph.json 마일스톤 id>", "started_at": "<ISO8601>" }
  ],
  "results": [
    { "project_id": "sodam-loop-eng", "milestone_id": "<id>", "outcome": "success|failure", "finished_at": "<ISO8601>", "note": "<120자 이내, S-4 마스킹 적용>" }
  ]
}
```

**M16 3개 기능과의 대응**:
- ① 읽음 회신 → `acks[]`. `judge.mjs`는 `acks[].read_scanned_at`을 자신이 마지막으로 발행한 `graph-state.json`의 `scanned_at`과 비교한다(`lib/refresh.mjs`의 `alreadyFresher()`와 동일한 시각-비교 패턴 재사용 — 새 잠금 메커니즘 불필요). 이 비교가 `01_PRD.md §5` 측정 2번("지목을 실제로 먼저 손댄 횟수")을 처음으로 자동 집계 가능하게 만든다.
- ② 결과 반영 → `results[]`. `outcome:"success"`인 항목은 `verified→done` 자동확정의 **새 증거 소스**가 된다. 🔴 단, **기존 `isHumanGated()`/`HUMAN_CONFIRM_RE` 차단을 그대로 통과해야 함** — 루프의 실행 사실도 "사람 확인" 문구가 박힌 마일스톤(M5 선례)을 우회하는 근거로 쓰지 않는다. 루프 회신은 *추가* 증거이지 *예외 통로*가 아니다.
- ③ 충돌 조정 → `executing[]`. 자동승격 로직은 승격 대상 마일스톤 id가 `executing[]`에 있고 아직 `results[]`에 대응 항목이 없으면 **판정을 보류**한다(승격도 거부도 아닌 "대기" — 그래프=판정, 루프=실행 역할 경계 유지).

**Graph-Eng 쪽 읽기 방어 규칙(신규 — 게이트 열리면 코드로 옮길 체크리스트)**:
- [ ] 이 파일은 **다른 저장소가 쓰는 미신뢰 외부 입력**이다 — 없거나 깨져도 `try/catch`로 흡수하고 **기존 동작을 그대로 유지**(acks/executing/results를 전부 빈 배열로 간주). 절대 throw로 판정 자체를 막지 않는다(S-6.5와 동일한 "가용성 우선" 원칙).
- [ ] 읽기 전 크기 상한 검사(신규 `LIMITS.MAX_ACK_BYTES`, `MAX_STATE_BYTES`와 같은 자릿수 제안) — 파싱 전에 걸러 대용량 파일로 인한 지연을 막는다(S-5.6과 동일 사상).
- [ ] `project_id`는 `graph.json`에 실제 존재하는 project id인지 확인 후에만 사용(M12 `canVerify()`가 이미 쓰는 "형제 경계 강제" 패턴 재사용) — 모르는 id나 스푸핑된 id는 조용히 무시.
- [ ] `note` 같은 자유 텍스트 필드는 커밋 제목과 동일하게 **S-4 마스킹 + 120자 자름**을 거친 뒤에만 `graph.json`의 `evidence`에 반영(다른 저장소발 텍스트라고 검사를 면제하지 않는다).
- [ ] `acked_at`/`started_at`/`finished_at`은 문자열 타입 검증만(파싱 실패 시 해당 항목 통째로 무시, 나머지 항목엔 영향 없음).

> ⚠️ **아직 결정 안 한 것(정직하게 남김)**: 정식 보안 규격 번호(`08_SECURITY_SPEC.md`의 S-6은 이미 6.1~6.5까지 쓰였음 — 이 신규 항목은 게이트가 실제로 열려 구현에 들어갈 때 `08_SECURITY_SPEC.md`에 정식 S-9 항목으로 편입하고 번호를 확정한다. 지금 번호를 박으면 그 사이 다른 개정이 끼어들 때 번호 충돌 위험이 있어 미룸). 소담루프엔지니어링 쪽 쓰기 구현 세부사항도 그 저장소가 결정할 몫으로 남겨둠.
