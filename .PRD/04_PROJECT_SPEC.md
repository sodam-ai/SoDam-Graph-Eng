...
├── LICENSE                  # ⚖️ Apache-2.0 전문 201줄 (발췌·수정 금지)
└── NOTICE                   # ⚖️ Copyright 2026 SoDam AI Studio + 상표 고지
...
- **이유 1**: 명령 9개(`graph-*`, 2026-08-20/21 M12·M13에서 `graph-verify`·`graph-block` 추가)가 이미 진입점이고, 세션 시작 훅이 자동 안내를 담당합니다. 스킬이 하는 일이 남아 있지 않습니다
...
1. **`SoDam-Agentic-Eng/docs/family-synergy.md`** — **정본** (**8,338바이트 · 2026-09-01 03:46 갱신됨**(2026-09-01 읽기전용 재확인), 소담에이전틱엔지니어링 소유)
   - 🔴 **읽기 직전에 크기·수정 시각을 다시 확인하십시오.** 이 PRD 작성 중에도 정본이 6,210B → 6,926B 로 바뀌었고, 2026-09-01 재확인 시점엔 8,338B로 더 늘어있었습니다(§3에 "위임 참여 현황" 절 추가 — 역할표·설치순서·공유인터페이스·설치표준은 불변). **낡은 정본 기준으로 M8 갱신안을 쓰면 또 어긋납니다.**
...
### 🟢 검증 완료 (2026-08-02) — **PRIVATE 저장소도 GitHub 마켓플레이스로 설치됩니다**

> **M0 `done_when` 8에서 실측했습니다.** 아래 두 형제의 상충하던 인식 중 **소담프롬프트엔지니어링 쪽이 맞았습니다.**
...
| 결론 | 내용 |
|------|------|
| **설치 방식** | 인증된 계정(`gh auth`) 기준 **HTTPS 클론**으로 처리됨. SSH 미설정이어도 자동 폴백 |
| **공개 전환** | 🟢 **불필요.** `09` §10 법무 게이트 **미발동 유지** |
| **로컬 폴더 방식** | 폴백으로 유지 (인증이 없는 다른 PC·다른 사용자 대비) |
| **패밀리 전달** | `07` §9 갱신안 8번 ③에 이 결과를 기록 — **형제 6곳도 공개 없이 GitHub 설치 가능** |

<details>
<summary>검증 전 상황 (기록 보존)</summary>

**추측으로 규격에 넣지 않았습니다.** 패밀리 안에서 **두 형제의 인식이 서로 달랐고, 검증 기록이 0건**이었습니다.
...
- [ ] **마켓플레이스 이름** — `sodamgraph-marketplace`(실측 다수파 3/6) 기본값으로 진행 중. 바꾸려면 `.claude-plugin/marketplace.json` 의 `name` **한 줄** (위 §마켓플레이스+플러그인 규격 참조)
- 그 외 프로젝트 스펙 관련 미결은 없습니다. **전체 미결 2건**(한글 명칭 · 마켓플레이스 이름)은 `01_PRD.md §9` 참조
  - 🔴 **14차 정정**: 여기 적혀 있던 *"3건 … 공개 범위"* 는 **낡은 표기**였습니다. **2026-08-02 M0 `done_when` 8 실측(PRIVATE 저장소 GitHub 설치 성공)으로 공개 범위는 해소**됐는데 `01`·`README` 만 고치고 이 줄을 놓쳤습니다 — 13차 98번이 고친 것과 **같은 유형의 재발**입니다
