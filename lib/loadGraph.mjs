...
const BLOCKER_KINDS = new Set(['human_test', 'legal', 'external_account', 'decision', 'purchase']);
...
/**
 * M13 — Blocker 개별 레코드 검증. 지금까지는 `blockers`가 "배열이어야 함"만
 * 검사되고 원소는 전혀 검증되지 않았다(빈 배열만 실운영에 있었기 때문에 드러나지
 * 않은 빈틈). `/graph-block`이 처음으로 이 배열에 실제로 쓰기 시작하므로,
 * 다른 엔티티(milestones·edges)와 동일한 수준으로 여기서도 S-7을 적용한다.
 */
function validateBlockers(g, milestoneIds) {
  const seen = new Set();
  for (const [i, b] of g.blockers.entries()) {
    const at = `blockers[${i}]`;
    if (typeof b.id !== 'string' || b.id.length === 0) {
      throw new GraphValidationError(`${at}.id`, '문자열이어야 함');
    }
    if (seen.has(b.id)) throw new GraphValidationError(`${at}.id`, `중복 id "${b.id}"`);
    seen.add(b.id);
    if (!milestoneIds.has(b.milestone_id)) {
      throw new GraphValidationError(`${at}.milestone_id`, `없는 마일스톤 "${b.milestone_id}"`);
    }
    if (!BLOCKER_KINDS.has(b.kind)) {
      throw new GraphValidationError(`${at}.kind`, `"${b.kind}" — 허용: ${[...BLOCKER_KINDS].join('·')}`);
    }
    if (typeof b.description !== 'string' || b.description.length === 0) {
      throw new GraphValidationError(`${at}.description`, '비어 있지 않은 문자열이어야 함');
    }
    if (!RE_DATE.test(b.since ?? '')) {
      throw new GraphValidationError(`${at}.since`, `"${b.since}" — YYYY-MM-DD`);
    }
  }
}
...
export function loadGraph(path = DEFAULT_GRAPH_PATH) {
  ...
  validateBlockers(g, milestoneIds);
  ...
  // M14: 운영용 이식성 오버라이드 — 콤마로 구분된 여러 경로. SODAM_GRAPH_ROOT(테스트 전용,
  // S-13.7)와는 별개 변수로 분리(S-13.9: "운영 기본값과 개발 오버라이드를 한 파일에 섞지
  // 않는다" — 같은 변수를 재사용하지 않고 새로 만듦). 존재 확인은 resolve 가 한다.
  const searchRootsOverride = process.env.SODAM_GRAPH_SEARCH_ROOTS;
  if (searchRootsOverride) {
    g.config.search_roots = searchRootsOverride
      .split(',')
      .map((p) => p.trim())
      .filter(Boolean)
      .map((p) => resolvePath(p));
  ...
  // 테스트 격리가 운영 오버라이드보다 항상 우선하도록 이 블록을 뒤에 둔다(둘 다 설정된
  // 경우 SODAM_GRAPH_ROOT 가 이긴다 — 테스트 중 실수로 운영 경로가 섞이면 안 되므로).
...
/**
 * M15 — 스키마 검증 게이트 (CLI 전용).
 * graph.json 이 손상되면 실행 전에 막고, 어디가 깨졌는지 한 줄로 표시한다.
 * loadGraph()가 이미 상세 메시지를 담아 throw 하므로(S-7 수용 기준), 여기서는
 * CLI 진입점 7곳이 각자 raw 스택 트레이스를 그대로 노출하던 걸 한 곳에서 막는다.
 */
export function loadGraphOrExit(path = DEFAULT_GRAPH_PATH) {
  try {
