...
import { loadGraph, loadGraphOrExit, DEFAULT_GRAPH_PATH, BLOCKER_KINDS } from './loadGraph.mjs';
...
import { canVerify, runVerifyCmd } from './verify.mjs';
 ...
 * @param {Array<{milestoneId:string, expectedFrom:string, toState:string, at:string, evidence?:string}>} updates
...
function applyMilestoneUpdates(updates) {
  ...
  const res = withLock(() => {
    ...
    for (const u of updates) {
      ...
      if (u.evidence !== undefined) m.evidence = u.evidence;
...
/**
 * M12 — verify_cmd 를 실제로 돌려 검증한다. `todo`·`doing` 상태에서만 `verified`
 * 로 올린다 — 이미 `verified` 이상(승격 파이프라인 진입)인 것은 M11 이 담당하는
 * 영역이라 여기서 건드리지 않는다(역할 분리).
 */
export function verifyMilestone(milestoneId) {
  const graph = loadGraph();
  const m = graph.milestones.find((x) => x.id === milestoneId);
  if (!m) return { ok: false, reason: `없는 마일스톤 "${milestoneId}"` };
  if (!canVerify(m)) {
    return { ok: false, reason: '자동 검증 대상이 아닙니다(자기 소유·배열형 verify_cmd 필수)' };
  }
  if (m.state !== 'todo' && m.state !== 'doing') {
    return {
      ok: false,
      reason: `이미 "${m.state}" 상태입니다 — 자동 검증은 todo/doing 에서만 verified 로 올립니다`,
    };
  }
  const result = runVerifyCmd(m);
  if (!result.ok) {
    return { ok: false, reason: `검증 실패 — ${result.reason} (${result.elapsedMs}ms)` };
  }
  const today = localDate(new Date());
  const evidence = `${today} verify_cmd 자동 실행 성공 (${result.elapsedMs}ms) · 명령: ${m.verify_cmd.join(' ')}`;
  const res = applyMilestoneUpdates([
    { milestoneId, expectedFrom: m.state, toState: 'verified', at: today, evidence },
  ]);
  if (!res.ok) return { ok: false, reason: res.reason };
  if (res.applied.length === 0) {
    return { ok: false, reason: '다른 세션에서 이미 상태가 바뀌었습니다 — 다시 시도하세요' };
  }
 ...
 * M13 — 사람 몫 블로커 기록/해제
 *
 * 규격: 02_DATA_MODEL.md §4 Blocker · 03_PHASES.md M13
 *
 * 🔴 왜 필요했나: `isHumanGated()`·E5(§evaluatePromotion)가 `graph.blockers`를
 *   읽기만 하고, 그 배열에 실제로 쓰는 명령이 하나도 없었다 — 운영 데이터가
 *   처음부터 끝까지 0건이라 두 안전장치가 사실상 죽은 코드였다. 이 절이
 *   처음으로 그 배열에 쓴다. milestones 와 마찬가지로 정본이므로 M11 과
 *   동일한 3원칙(락·최신 재로드·검증 우선)을 그대로 따른다.
 *
 * 🔴 해제 경로가 반드시 필요한 이유: PRD 어디에도 Blocker를 없애는 방법이
 *   정의돼 있지 않다. 없이 만들면 배열이 append-only로 계속 쌓이고, 이미
 *   풀린 블로커가 `isHumanGated()`의 자동승격을 영구히 막는 새 정체를
 *   이 도구 자신이 만들어낸다 — 그래서 기록과 해제를 같은 라운드에 만든다.
 * ══════════════════════════════════════════════════════════════ */

/** 기존 `blk-NNN` id 중 가장 큰 번호+1 (삭제로 생긴 번호 공백에도 안전) */
...
/** `/graph-block` — 마일스톤에 사람 몫 블로커를 기록한다. */
export function blockMilestone(milestoneId, kind, description) {
  if (!BLOCKER_KINDS.has(kind)) {
    return { ok: false, reason: `허용되지 않는 kind "${kind}" — ${[...BLOCKER_KINDS].join('·')} 중 하나` };
  }
  if (typeof description !== 'string' || description.trim().length === 0) {
    return { ok: false, reason: 'description 이 비어 있습니다' };
  }
  const res = withLock(() => {
    const raw = JSON.parse(readFileSync(DEFAULT_GRAPH_PATH, 'utf8'));
    if (!raw.milestones.some((m) => m.id === milestoneId)) {
      return { rejected: `없는 마일스톤 "${milestoneId}"` };
    }
    const blocker = {
      id: nextBlockerId(raw.blockers),
      milestone_id: milestoneId,
      kind,
      description: description.trim(),
      since: localDate(new Date()), // 추측 안 함 — 기록 시점 오늘 날짜 자동 기입
    };
    raw.blockers.push(blocker);
    safeWriteAtomic(DEFAULT_GRAPH_PATH, `${JSON.stringify(raw, null, 2)}\n`);
    return { blocker };
  });
  if (!res.ok) return { ok: false, reason: res.reason };
  if (res.value.rejected) return { ok: false, reason: res.value.rejected };
  return { ok: true, blocker: res.value.blocker };
}

/** `/graph-block --resolve` — 풀린 블로커를 제거한다. */
export function resolveBlocker(blockerId) {
  const res = withLock(() => {
    const raw = JSON.parse(readFileSync(DEFAULT_GRAPH_PATH, 'utf8'));
    const idx = raw.blockers.findIndex((b) => b.id === blockerId);
    if (idx === -1) return { removed: false };
    const [removed] = raw.blockers.splice(idx, 1);
    safeWriteAtomic(DEFAULT_GRAPH_PATH, `${JSON.stringify(raw, null, 2)}\n`);
    return { removed: true, blocker: removed };
  });
  if (!res.ok) return { ok: false, reason: res.reason };
  if (!res.value.removed) return { ok: false, reason: `없는 블로커 id "${blockerId}"` };
  return { ok: true, blocker: res.value.blocker };
}
 ...
 *      node lib/judge.mjs --verify <마일스톤ID>                   → M12 자동 검증
 *      node lib/judge.mjs --block <마일스톤ID> --kind <종류> --description "..."  → M13 블로커 기록
 *      node lib/judge.mjs --resolve-block <블로커ID>              → M13 블로커 해제
 */
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  ...
  const resolveBlockIdx = argv.indexOf('--resolve-block');

  if (blockIdx >= 0) {
    const id = argv[blockIdx + 1];
    const kindIdx = argv.indexOf('--kind');
    const descIdx = argv.indexOf('--description');
    const kind = kindIdx >= 0 ? argv[kindIdx + 1] : undefined;
    const description = descIdx >= 0 ? argv[descIdx + 1] : undefined;
    if (!id || id.startsWith('--') || !kind || !description) {
      console.log(
        `사용법: node lib/judge.mjs --block <마일스톤ID> --kind <${[...BLOCKER_KINDS].join('|')}> --description "..."`
      );
      process.exit(0);
    }
    const r = blockMilestone(id, kind, description);
    console.log(r.ok ? `기록 완료 — ${r.blocker.id} (${r.blocker.since}부터, ${id})` : `실패 — ${r.reason}`);
    process.exit(0);
  }

  if (resolveBlockIdx >= 0) {
    const id = argv[resolveBlockIdx + 1];
    if (!id || id.startsWith('--')) {
      console.log('사용법: node lib/judge.mjs --resolve-block <블로커ID>');
      process.exit(0);
    }
    const r = resolveBlocker(id);
    console.log(r.ok ? `해제 완료 — ${id} (${r.blocker.milestone_id})` : `실패 — ${r.reason}`);
    process.exit(0);
  }
  ...
  if (verifyIdx >= 0) {
    ...
    if (!id || id.startsWith('--')) {
      console.log('사용법: node lib/judge.mjs --verify <마일스톤ID>');
      process.exit(0);
    }
    const r = verifyMilestone(id);
    console.log(
      r.ok ? `검증 완료 — ${id} 는 verified 로 확정됐습니다 (${r.elapsedMs}ms).` : `실패 — ${r.reason}`
    );
    process.exit(0);
  }
